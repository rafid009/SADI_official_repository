...
import matplotlib
import pickle
from datasets.synthetic_data import create_synthetic_data_v1, feats_v4
import json
...
num_seasons = 50
noise = False
train_loader, valid_loader, mean, std = get_dataloader(n_steps, n_features, num_seasons, batch_size=32, missing_ratio=0.1, seed=seed, is_test=False, v2='v4', noise=noise)
...
filename = f"model_SADI_synth_v4_{name}_new.pth"
print(f"\n\SADI training starts.....\n")
model_folder = "saved_model_v4"
...
mse_folder = f"results_synth_v4_{name}_{partial_bm_config['features']}/metric"
data_folder = f"results_synth_v4_{name}_{partial_bm_config['features']}/data"

pbm = [1,2,3,4,5]
for bm in pbm:
    partial_bm_config['features'] = bm
    print(f"partial features: {partial_bm_config['features']}")
    evaluate_imputation_all(models=models, trials=1, mse_folder=mse_folder, dataset_name='synth_v4', batch_size=32, mean=mean, std=std, partial_bm_config=partial_bm_config)
...
print(f"\nForecasting:")
evaluate_imputation_all(models=models, trials=1, mse_folder=mse_folder, dataset_name='synth_v4', batch_size=32, length=(10, 80), forecasting=True, noise=noise, mean=mean, std=std)
